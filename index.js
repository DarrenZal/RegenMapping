var $hWkPe$threespritetext = require("three-spritetext");
...
class $60c2e97ffb936cec$var$RegenMappingApp {
    constructor(){
        this.graph = null;
    ...
    async loadProfiles() {
        ...
        if (Object.keys(this.profiles).length === 0) throw new Error('No profiles could be loaded from any source');
    }
    async loadFromMurmurationsIndex() {
        ...
        for (const queryUrl of queries)try {
            console.log(`\u{1F50D} Querying: ${queryUrl}`);
            const response = await fetch(queryUrl);
            if (response.ok) {
                const result = await response.json();
                console.log(`\u{1F4CA} Found ${result.data?.length || 0} profiles in total`);
                if (result.data && result.data.length > 0) {
                    // Filter for our specific profiles
                    const ourProfiles = result.data.filter((node)=>ourProfileUrls.some((url)=>node.profile_url.includes('DarrenZal/RegenMapping')));
                    console.log(`\u{1F4CA} Found ${ourProfiles.length} of our profiles`);
                    // Load our filtered profiles
                    for (const node of ourProfiles)try {
                        console.log(`\u{1F50D} Loading profile from: ${node.profile_url}`);
                        ...
                        if (profileResponse.ok) {
                            ...
                            console.log(`\u{2705} Loaded from Murmurations: ${profile.name}`);
                        }
                    } catch (profileError) {
        ...
        console.log(`\u{1F389} Total profiles loaded: ${Object.keys(this.profiles).length}`);
    }
    async loadFromGitHub() {
        ...
        for (const url of profileUrls)try {
            ...
            if (response.ok) {
                ...
                console.log(`\u{2705} Loaded from GitHub: ${profile.name}`);
            }
        } catch (error) {
    ...
    async convertToUnified(murmProfile) {
        if (!this.cambriaReady) throw new Error('Cambria lenses not loaded - cannot convert to unified format');
    ...
    async convertToSchemaOrg(murmProfile) {
        if (!this.cambriaReady) throw new Error('Cambria lenses not loaded - cannot convert to Schema.org format');
    ...
    setupGraph() {
        ...
        this.graph = ForceGraph3D()(document.getElementById('graph')).graphData(graphData).nodeLabel((node)=>{
            // Only show hover labels for non-active nodes
            if (node.id === this.expandedNode || node.id === this.activeSchemaNode) return null; // Active nodes will use nodeThreeObject for labels
            return node.name;
        ...
        .nodeThreeObject((node)=>{
            ...
            if (node.id === this.expandedNode || node.id === this.activeSchemaNode) {
                ...
                const sprite = new (0, ($parcel$interopDefault($hWkPe$threespritetext)))(node.name);
                sprite.color = 'white';
                sprite.textHeight = 8;
        ...
        }).nodeColor((node)=>this.getNodeColor(node)).nodeVal((node)=>node.size || 4).linkColor((link)=>this.getLinkColor(link)).linkWidth(2).linkLabel((link)=>link.label || '').linkDirectionalArrowLength((link)=>link.type && link.type !== 'schema-link' && link.type !== 'lens' ? 3 : 0).linkDirectionalArrowRelPos(0.8) // Position arrows closer to target to avoid overlap
        .linkDirectionalArrowColor(()=>'rgba(255, 255, 255, 0.8)').backgroundColor('rgba(0,0,0,0)').onNodeClick(this.handleNodeClick.bind(this)).onNodeDragEnd((node)=>{
            node.fx = node.x;
            node.fy = node.y;
            node.fz = node.z;
        });
    ...
    generateGraphData() {
        ...
        Object.keys(this.profiles).forEach((profileId)=>{
            ...
            relationships.forEach((rel)=>{
                ...
                const targetProfileId = Object.keys(this.profiles).find((id)=>this.profiles[id].murmurations.name === rel.target);
                if (targetProfileId) {
    ...
    getNodeColor(node) {
        if (node.type === 'schema') {
            ...
            if (node.id === this.activeSchemaNode) return '#2ecc71'; // Green for active schema
            else return '#bdc3c7'; // Light gray for inactive schemas
        }
        if (node.type === 'profile') {
            // Determine if this is an organization or person
            const isOrganization = node.profile && node.profile.murmurations.linked_schemas && node.profile.murmurations.linked_schemas.some((schema)=>schema.includes('organizations_schema'));
            if (node.id === this.expandedNode) // Active profile colors - dark blue for both people and organizations
            return '#2980b9'; // Dark blue for active profiles
            else // Inactive profile colors - light blue for both people and organizations
    ...
    getLinkColor(link) {
        ...
        if (link.type === 'lens' && this.activeSchemaNode) {
            ...
            if (link.target.id === this.activeSchemaNode || link.source.id === this.activeSchemaNode) return '#2ecc71'; // Green flow to/from active schema
        }
        const colors = {
            'member': '#2ecc71',
            'advisor': '#3498db',
    ...
    handleNodeClick(node) {
        if (node.type === 'schema') this.handleSchemaNodeClick(node);
        else if (node.type === 'profile') this.handleProfileNodeClick(node);
    }
    handleProfileNodeClick(node) {
        if (this.expandedNode === node.id) // Collapse the node
        this.collapseNode();
    ...
    handleSchemaNodeClick(schemaNode) {
        ...
        if (this.currentProfile) this.displayProfile(this.currentProfile, this.currentSchema);
    }
    expandNode(node) {
    ...
    collapseNode() {
        ...
        const filteredNodes = currentData.nodes.filter((n)=>!this.schemaNodes.some((sn)=>sn.id === n.id));
        const filteredLinks = currentData.links.filter((l)=>!this.schemaNodes.some((sn)=>sn.id === l.source.id || sn.id === l.target.id) && l.type !== 'schema-link' && l.type !== 'lens');
        this.graph.graphData({
            nodes: filteredNodes,
            links: filteredLinks
        });
    ...
    displayProfile(profile, schema) {
        ...
        if (schema === 'murmurations') html += this.renderMurmurationsFields(data);
        else if (schema === 'unified') html += this.renderUnifiedFields(data);
        else if (schema === 'schemaorg') html += this.renderSchemaOrgFields(data);
    ...
    renderMurmurationsFields(data) {
        let html = '';
        if (data.primary_url) html += this.renderField('Primary URL', `<a href="${data.primary_url}" target="_blank">${data.primary_url}</a>`);
        if (data.locality) html += this.renderField('Location', `${data.locality}, ${data.region || ''} ${data.country_name || ''}`.trim());
        if (data.tags && data.tags.length > 0) html += this.renderField('Tags', data.tags.join(', '));
        if (data.geolocation) html += this.renderField('Coordinates', `${data.geolocation.lat}, ${data.geolocation.lon}`);
        if (data.relationships && data.relationships.length > 0) {
            const relationshipsHtml = data.relationships.map((rel, index)=>`<div class="relationship-item">
    ...
    renderUnifiedFields(data) {
        let html = '';
        if (data['murm:primary_url']) html += this.renderField('Primary URL', `<a href="${data['murm:primary_url']}" target="_blank">${data['murm:primary_url']}</a>`);
        if (data['regen:locality']) html += this.renderField('Locality', data['regen:locality']);
        if (data['regen:domainTags'] && data['regen:domainTags'].length > 0) html += this.renderField('Domain Tags', data['regen:domainTags'].join(', '));
        if (data['@type']) html += this.renderField('Type', Array.isArray(data['@type']) ? data['@type'].join(', ') : data['@type']);
        if (data['regen:relationships'] && data['regen:relationships'].length > 0) {
            const relationshipsHtml = data['regen:relationships'].map((rel)=>`<div class="relationship-item">
                    <strong>${rel.type}</strong>: <a href="#" class="profile-link" data-target="${rel.target}">${rel.target}</a>
                    <br><em>${rel.description}</em>
                </div>`).join('');
            html += this.renderField('Relationships', relationshipsHtml);
    ...
    renderSchemaOrgFields(data) {
        let html = '';
        if (data.url) html += this.renderField('URL', `<a href="${data.url}" target="_blank">${data.url}</a>`);
        ...
        if (data['@type']) html += this.renderField('Schema Type', data['@type']);
        // Add profile source link
        html += this.renderProfileSource(data.name);
        return html;
    }
    ...
    renderProfileSource(profileName) {
        console.log(`\u{1F50D} renderProfileSource called with name: "${profileName}", schema: ${this.currentSchema}`);
        ...
        for(const id in this.profiles){
            ...
            console.log(`  \u{2022} ID: ${id}`);
            console.log(`    \u{2022} Murmurations name: "${profile.murmurations.name}"`);
            console.log(`    \u{2022} Unified name: "${profile.unified.name}"`);
            console.log(`    \u{2022} SchemaOrg name: "${profile.schemaorg.name}"`);
        }
        ...
        if (!currentProfileId) {
            console.log(`\u{274C} No matching profile found for name: "${profileName}"`);
        ...
        if (this.currentSchema === 'murmurations') {
            ...
            if (murmurationsName === 'Dylan Tull') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/murmurations/murm-person-dylan-tull.json';
            else if (murmurationsName === 'Dr. Karen O\'Brien') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/murmurations/murm-person-karen-obrien.json';
            else if (murmurationsName === 'Global Regenerative Cooperative') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/murmurations/murm-org-global-regenerative-coop.json';
            else // Generic case
            ...
            if (murmurationsName === 'Dylan Tull') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/unified/regen-person-dylan-tull.jsonld';
            else if (murmurationsName === 'Dr. Karen O\'Brien') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/unified/regen-person-karen-obrien.jsonld';
            else if (murmurationsName === 'Global Regenerative Cooperative') githubUrl = 'https://raw.githubusercontent.com/DarrenZal/RegenMapping/main/profiles/unified/regen-org-global-regenerative-coop.jsonld';
            else // Generic case
        ...
        } else return this.renderField('Profile Source', 'Source information not available');
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e)=>{
            if (e.target.classList.contains('profile-link')) {
                ...
                if (targetName) this.navigateToProfile(targetName);
            }
        });
    }
    updateSchemaButtons() {
    ...
    updateGraphColors() {
        ...
        if (this.graph) this.graph.nodeColor((node)=>this.getNodeColor(node)).linkColor((link)=>this.getLinkColor(link));
    }
    openSidebar() {
        document.getElementById('sidebar').classList.add('open');
    }
    navigateToProfile(profileName) {
        console.log(`\u{1F50D} Navigating to profile: "${profileName}"`);
        // Debug: List all available profile names
        const availableNames = Object.keys(this.profiles).map((id)=>`${id}: "${this.profiles[id].murmurations.name}"`);
        console.log("\uD83D\uDCCB Available profiles:", availableNames);
        // Find the profile by name
        const profileId = Object.keys(this.profiles).find((id)=>this.profiles[id].murmurations.name === profileName);
        console.log(`\u{1F3AF} Found profile ID: ${profileId}`);
        if (profileId) {
            // Find the node in the graph
            const currentData = this.graph.graphData();
            const targetNode = currentData.nodes.find((node)=>node.id === profileId);
            console.log(`\u{1F4CD} Found target node:`, targetNode ? targetNode.name : 'Not found');
            if (targetNode) {
            ...
            } else console.error(`\u{274C} Target node not found for profile ID: ${profileId}`);
        } else console.error(`\u{274C} Profile not found for name: "${profileName}"`);
    }
    hideLoading() {
    ...
    showError(message) {
        ...
        graphContainer.innerHTML = `
            <div class="loading">
                <p style="color: #e74c3c;">\u{274C} ${message}</p>
            </div>
        `;
    }
}
...
let $60c2e97ffb936cec$var$app;
// Initialize the app when the page loads
document.addEventListener('DOMContentLoaded', ()=>{
    $60c2e97ffb936cec$var$app = new $60c2e97ffb936cec$var$RegenMappingApp();
});
